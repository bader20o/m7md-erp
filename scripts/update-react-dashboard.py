...
filepath = './frontend/src/pages/admin/Analytics.js'
with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# Replace setLoadingState
def repl_loading(m):
...
content = re.sub(r"function setLoadingState\(\) \{.*?function populateOverview", repl_loading, content, flags=re.DOTALL)

# Replace populateOverview logic
def repl_populate(m):
    inner = m.group(1)
    injection = r"""
      ...
      if (statusDistEl && window.SimpleDonutChart) {
        ...
        statusDistEl.innerHTML = items.length 
      ...
      if (revChartEl && window.SimpleBarChart) {
      ...
      if (waitingEl) {
         ...
         waitingEl.innerHTML = waiting.length
            ? waiting.map(w => '<div class="flex items-center justify-between py-2 border-b border-white/5 last:border-0">' +
                  ...
                  '</div>' +
                  '<div class="text-xs text-danger font-bold">' + Math.round(w.waitTimeMin) + ' mins wait</div>' +
                '</div>').join('')
            : '<div class="text-sm text-muted">No waiting customers.</div>';
      }

      const ptChartEl = document.getElementById('chart-profit-trend');
      if (ptChartEl && window.SimpleBarChart) {
...
"""
    return "function populateOverview(data) {" + inner + injection + "\n    }\n\n    function populateExecutiveSummary"

content = re.sub(r"function populateOverview\(data\) \{(.*?)function populateExecutiveSummary", repl_populate, content, flags=re.DOTALL)

# Replace Return statement entirely.
# We will match the very last return statement which returns the huge UI literal string.
return_pattern = r"return `(.*?)`;\n}"
replacement_ui = r"""return `<div class="w-full max-w-7xl mx-auto flex flex-col gap-6">
...
content = re.sub(return_pattern, replacement_ui, content, flags=re.DOTALL)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)

print("Safely replaced Analytics template")
