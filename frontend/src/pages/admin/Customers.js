  ...
  window.onMount = () => {
          ...
          tbody.innerHTML = items
            ...
            <tr class="border-b border-border hover:bg-bg cursor-pointer transition-colors" onclick="window.openCustomerDetails('${item.id}')">
              ...
              <td class="px-4 py-3 text-sm text-center whitespace-nowrap">
                <span class="inline-flex min-w-[3.5rem] items-center justify-center rounded bg-bg px-2 py-0.5 text-xs font-semibold tabular-nums whitespace-nowrap border border-border">
    ...
    async function loadCustomerDetails(id) {
      ...
      try {
        ...
        const loyaltyProgressHtml =
          loyaltyProgressRows.length > 0
            ? loyaltyProgressRows
                .map(
                  (entry) => `
                  <div class="rounded-lg border border-border p-3">
                    <div class="text-xs font-semibold text-text">${entry.rewardRule?.title || "Reward Rule"}</div>
                    <div class="mt-1 text-xs text-muted">Progress ${entry.progressValue} / ${entry.rewardRule?.triggerValue || "-"}</div>
                    <div class="mt-1 text-xs text-muted">Completed Cycles: ${entry.completedCycles}</div>
                  </div>
                `
                )
                .join("")
            : `<div class="rounded-lg border border-dashed border-border p-3 text-xs text-muted">No loyalty progress yet.</div>`;
        const loyaltyAvailableHtml =
          loyaltyAvailableRows.length > 0
            ? loyaltyAvailableRows
                .slice(0, 5)
                .map(
                  (entry) => `
                  <div class="rounded-lg border border-border p-3 text-xs">
                    <div class="font-semibold text-text">${entry.rewardRule?.title || entry.rewardType}</div>
                    <div class="text-muted mt-1">Issued ${formatDate(entry.issuedAt)}</div>
                  </div>
                `
                )
                .join("")
            : `<div class="rounded-lg border border-dashed border-border p-3 text-xs text-muted">No available rewards.</div>`;
        const loyaltyHistoryHtml =
          loyaltyHistoryRows.length > 0
            ? loyaltyHistoryRows
                ...
                .map(
                  ...
                  <div class="rounded-lg border border-border p-3 text-xs">
                    <div class="font-semibold text-text">${entry.rewardRule?.title || entry.rewardType}</div>
                    <div class="text-muted mt-1">${entry.status} - ${formatDate(entry.redeemedAt || entry.issuedAt)}</div>
                  </div>
                `
                )
                .join("")
            : `<div class="rounded-lg border border-dashed border-border p-3 text-xs text-muted">No reward history.</div>`;
        ...
        content.innerHTML = `
          ...
          <div id="customer-tab-overview" class="customer-tab-content space-y-5 animate-in fade-in">
            ...
            <h4 class="text-sm font-bold mt-4">Loyalty & Rewards</h4>
            <div class="grid grid-cols-2 gap-3 text-xs">
              <div class="rounded-lg border border-border p-3">
                <div class="text-muted uppercase">Valid Visits</div>
                <div class="text-base font-bold mt-1">${loyalty.totalValidVisits ?? 0}</div>
              </div>
              <div class="rounded-lg border border-border p-3">
                <div class="text-muted uppercase">Completed Services</div>
                <div class="text-base font-bold mt-1">${loyalty.completedServicesCount ?? 0}</div>
              </div>
            </div>
            <div class="mt-3 space-y-2">
              <div class="text-xs font-semibold text-muted uppercase">Active Progress</div>
              <div class="grid gap-2">${loyaltyProgressHtml}</div>
            </div>
            <div class="mt-3 space-y-2">
              <div class="text-xs font-semibold text-muted uppercase">Available Rewards</div>
              <div class="grid gap-2">${loyaltyAvailableHtml}</div>
            </div>
            <div class="mt-3 space-y-2">
              <div class="text-xs font-semibold text-muted uppercase">Reward History</div>
              <div class="grid gap-2">${loyaltyHistoryHtml}</div>
            </div>
          </div>
