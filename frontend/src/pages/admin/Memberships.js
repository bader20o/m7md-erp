...
const PLAN_DURATION_OPTIONS = [1, 3, 6, 12, 24];
...
function resolveLocalizedText(lang, enValue, arValue, fallback = '') {
  return lang === 'ar'
    ? (String(arValue || '').trim() || String(enValue || '').trim() || fallback)
    : (String(enValue || '').trim() || String(arValue || '').trim() || fallback);
}
  ...
  window.onMount = async () => {
    ...
    function syncTabs() {
      tabs.forEach((button) => {
        ...
        button.className = `px-4 py-2 rounded-xl text-sm font-bold transition-colors ${isActive
          ? 'bg-primary text-white'
          : 'bg-bg border border-border text-muted hover:text-text hover:border-primary/40'
          }`;
      });
    ...
    function renderSubscribers() {
      ...
      statusButtons.forEach((button) => {
        ...
        button.className = `px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${active
          ? 'bg-primary text-white'
          : 'bg-bg border border-border text-muted hover:text-text hover:border-primary/40'
          }`;
      });
        ...
        form.addEventListener('submit', async (event) => {
          ...
          try {
            const response = await apiFetch(`/admin/memberships/subscriptions/${item.id}/benefits/${benefitId}/confirm`, {
              method: 'POST',
              body: {
                confirmNote: noteField?.value || undefined
              }
            });
      ...
      subscriberModalContent.querySelector('#approve-subscription-form')?.addEventListener('submit', async (event) => {
        ...
        try {
          const response = await apiFetch(`/admin/memberships/subscriptions/${item.id}/approve`, {
            method: 'POST',
            body: {
              deliveryCompanyName: form.deliveryCompanyName.value || undefined,
              deliveryPhone: form.deliveryPhone.value || undefined,
              deliveryTrackingCode: form.deliveryTrackingCode.value || undefined,
              deliveryNote: form.deliveryNote.value || undefined
            }
          });
          state.currentSubscriberDetail = response.item;
          renderSubscriberDetail();
    ...
    function openPlanModal(plan = null) {
      ...
      function updateSubmitState() {
        ...
        submitButton.textContent = isSavingPlan
          ? 'Saving...'
          : (state.editingPlanId ? 'Save Changes' : 'Save Plan');
      }

      function showImage(url) {
        imagePreview.innerHTML = url
          ? `<img src="${url}" alt="Plan image" class="w-full h-full object-cover">`
              ...
              <div class="mt-5 rounded-2xl border border-border bg-bg p-4">
                ...
                <div class="mt-3 space-y-3">
                  ${previewBenefits.length
                    ...
                    `).join('')
                    : '<div class="text-sm text-muted">No benefits configured yet.</div>'}
                </div>
              </div>
            </div>
          </div>
      ...
      function syncBenefitValidationUi() {
        ...
        benefitsEditor.querySelectorAll('[data-benefit-index]').forEach((node) => {
          ...
          const message = status.duplicateCode
            ? 'Benefit code must be unique within this plan.'
            : (shouldShowBenefitValidation && status.missingCode)
      ...
      function renderPlanBenefitsEditor() {
        const validation = getBenefitValidation();
        benefitsEditor.innerHTML = state.planBenefits.length
          ? state.planBenefits.map((benefit, index) => {
            const errorMessage = validation[index]?.duplicateCode
              ? 'Benefit code must be unique within this plan.'
              : (shouldShowBenefitValidation && validation[index]?.missingCode)
                ? 'Benefit code is required.'
                : '';
      ...
      planForm.addEventListener('submit', async (event) => {
        ...
        const payload = {
          ...
          imageUrl: planForm.imageUrl.value || undefined,
          price: Math.max(0, parseFloat(planForm.price.value)),
          durationMonths: parseInt(planForm.durationMonths.value, 10),
          descriptionEn: planForm.descriptionEn.value || undefined,
          descriptionAr: planForm.descriptionAr.value || undefined,
          benefits: state.planBenefits.map((benefit) => ({
            ...(benefit.id ? { id: benefit.id } : {}),
            code: String(benefit.code || '').trim(),
            titleEn: benefit.titleEn,
            titleAr: benefit.titleAr,
            descriptionEn: benefit.descriptionEn || undefined,
            descriptionAr: benefit.descriptionAr || undefined,
    ...
    async function loadSubscribers() {
      ...
      try {
        const response = await apiFetch(`/admin/memberships/subscriptions${buildQuery({
          ...
          q: state.subscriberQuery || undefined
        })}`);
