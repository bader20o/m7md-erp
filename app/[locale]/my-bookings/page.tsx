        ...
        {bookings.map((booking) => (
          <article key={booking.id} className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className={`rounded-full px-2 py-1 text-xs font-medium ${statusClass[booking.status]}`}>{booking.status}</span>
            </div>
            ...
            <CustomerBookingActions bookingId={booking.id} status={booking.status} hasReview={Boolean(booking.review)} />
          </article>
        ))}
      </div>
    </div>
